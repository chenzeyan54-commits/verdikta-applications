...
contract MockVerdiktaAggregator {
    ...
    function requestAIEvaluationWithApproval(
        ...
        // Mirror the keeper's checks so tests prove the escrow never sends something the
        // real aggregator would reject.
        uint256 eff = _maxFee < maxOracleFee ? _maxFee : maxOracleFee;
        require(_estimatedBaseCost < eff, "Base cost must be less than max fee");
        require(_maxFeeBasedScalingFactor >= 1, "Max scaling factor must be at least 1");
        require(msg.value == eff * feeMultiplier, "InsufficientPayment");
        _nonce++;
    ...
    /// @dev Mirrors the real aggregator: the requested fee is clamped to the ceiling.
    function maxTotalFee(uint256 requestedMaxOracleFee) external view returns (uint256) {
        ...
        return eff * feeMultiplier;
    }

    // ethOwed(address) getter is auto-generated by the public `ethOwed` mapping.

    bool public withdrawBroken;   // test switch: make withdrawEth revert (models an aggregator change)
    bool public withdrawBurnsGas; // test switch: make withdrawEth consume ALL forwarded gas
    function setWithdrawBroken(bool v) external { withdrawBroken = v; }
    function setWithdrawBurnsGas(bool v) external { withdrawBurnsGas = v; }

    function withdrawEth() external {
        require(!withdrawBroken, "aggregator withdraw disabled");
        if (withdrawBurnsGas) { uint256 x; while (true) { x = x + 1; } }
