...
- `MAX_SUBMISSIONS_PER_BOUNTY = 128`; payout gas cap `PAYOUT_GAS_LIMIT = 120000` and inline-refund gas cap `INLINE_REFUND_GAS_LIMIT = 200000`; CID shape validation; creator-owned oracle settings; leftover prepay refunded to the funder.
...
The same-hunter exemption is deliberately narrower for `PendingVerdikta`. The helper takes a `forCreatorApproval` flag: a same-hunter sibling that is **in evaluation** blocks `creatorApproveSubmission` (`earlier submission unresolved`) but not the hunter's own `finalizeSubmission`. Reason: an in-flight (possibly already passing) evaluation is the hunter's live claim to `arbiterDeterminationPayment`; if the creator could approve a newer version for `creatorDeterminationPayment` (which may be far smaller) the bounty would become Awarded and the earlier version would finalize to `PassedUnpaid`, voiding a payout the hunter already paid the oracle to earn. Finalize of the newer version by the hunter is harmless (same payee, same rate). The creator may approve the revision after the earlier evaluation has resolved as failed; nobody is forced to pay a prepay.
...
**Submission cap.** `MAX_SUBMISSIONS_PER_BOUNTY = 128` (public constant). `prepareSubmission` reverts `submission limit reached` once a bounty has 128 submissions in any state, counting every hunter. This bounds the per-bounty scans at start, finalize and creator approval; measured worst case at the cap (127 started siblings with results) is about 5.6M gas per finalize or start, so a passing submission can always be finalized. Without the cap, roughly 11,700 gas-only junk prepares (about 7.4B gas) would have pushed a passing submission's finalize past the block gas limit and locked both the payout and the escrow forever. A full bounty only rejects new prepares: existing submissions still resolve, and the creator can still close at the deadline. Targeted bounties are effectively immune to junk since only the target can prepare.
